...
module.exports = function (RED) {
	const coreFunction = async function (msg, config) {
		...
		try {
			...
			handleResponse(node, msg, response, 'Field created', 'Failed to create field', (response) => {
				const enhancedData = {
			...
			handleError(node, msg, 'Failed to create field', error.message);
		}
	};

	/**
	 * KlickTippFieldCreateNode - A Node-RED node to create a new manual field.
	 ...
	 */
	function KlickTippFieldCreateNode(config) {
		RED.nodes.createNode(this, config);
		const node = this;
		createKlickTippSessionNode(RED, node, coreFunction)(config);
	}

	RED.nodes.registerType('klicktipp-field-create', KlickTippFieldCreateNode);
};
